...
ui("do_Button_gif1").on("touch", function() {
	...
	// repeat默认值为1执行一次动画效果，小于0时表示无限循环，为0时表示没有动画效果,大于0表示动画效果重复的次数；windows平台不支持设置重复次数，只能循环播放
...
ui("do_Button_image1").on("touch", function() {
	...
	// 开始动画,支持source://路径下的图片
	do_FrameAnimationView.startImages({
		data : imgs,
		repeat : -1
	}); // repeat默认值为1执行一次动画效果，小于0时表示无限循环，为0时表示没有动画效果,大于0表示动画效果重复的次数
})

ui("do_Button_image2").on(
		"touch",
		function() {
			...
			if (!sm("do_Storage").fileExist("data://gif/image/12.png")) { // data://gif/image/下不存在图片，先拷贝图片
